...
export function PortalRecordActionInstructions({ 
  ...
  // Use red banner for both conflict and unmatched records that need attention
  const needsAttention = portalRecord.matchType === 'Conflict' || portalRecord.matchType === 'Unmatched';
  const bannerStyle = needsAttention 
    ? "bg-red-50 border border-red-200" 
    : "bg-[#EBF1FF] border border-[#C7D9FF]";
  const iconStyle = needsAttention ? "text-red-600" : "text-[#253EA7]";
  const textStyle = needsAttention ? "text-red-700" : "text-[#253EA7]";
