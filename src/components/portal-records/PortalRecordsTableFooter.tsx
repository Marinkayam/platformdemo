...
export function PortalRecordsTableFooter({ records }: PortalRecordsTableFooterProps) {
  ...
  return (
    <tfoot>
      <tr className="bg-[#F6F7F9] hover:bg-[#F6F7F9] border-t border-gray-200">
        <td colSpan={2} className="font-semibold text-gray-700 sticky left-0 z-10 bg-[#F6F7F9] border-r border-gray-200 px-4 py-3">
          <div className="flex items-center gap-2">
            <span>Total Portal Records:</span>
            <span className="font-bold text-gray-900">{totalRecords}</span>
          </div>
        </td>
        <td colSpan={2} className="bg-[#F6F7F9] px-4 py-3"></td>
        <td className="font-semibold text-gray-700 bg-[#F6F7F9] px-4 py-3">
          <div className="flex items-center gap-2 whitespace-nowrap">
            <span>Total Amount:</span>
            <span className="font-bold text-gray-900 whitespace-nowrap">{formatTotalAmounts()}</span>
          </div>
        </td>
        <td colSpan={6} className="bg-[#F6F7F9] px-4 py-3"></td>
      </tr>
    </tfoot>
  );
}
